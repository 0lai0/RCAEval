...
def process(data_path):
    ...
    try:
        ...
        if args.method == "gnn_kan_rca":
            ...
            fault_type = metric  # Parsed from directory name: cpu, mem, disk, socket, delay, loss
            ...
            out = func(
                ...
                sparsity_lambda=2e-3,           # Stronger sparsity regularization
                # High-precision training parameters
                learning_rate=(args.learning_rate if args.learning_rate is not None else 2.856334749157e-8),  # Tuned learning rate
                num_epochs=400,                 # Larger number of training epochs
                kan_grid_size=20,               # Larger KAN grid size
                hidden_dim=128,                 # Larger hidden dimension
                target_feature_dim=128,         # Larger target feature dimension
                similarity_threshold=0.265,     # Lower similarity threshold
                max_edges_per_node=15,          # Allow more edges per node
                gradient_clipping=0.6,          # Stricter gradient clipping
                # Supervised learning related parameters
                fault_type=fault_type,          # Fault type information (critical)
                enhanced_contrast=True,         # Enable enhanced contrastive learning
                fault_type_aware=True,          # Enable fault-type-aware modeling
                adaptive_learning=True,         # Enable adaptive learning
                multi_scale_features=True,      # Enable multi-scale features
                temporal_attention=True,        # Enable temporal attention
                # Basis function choice
                basis_function=args.basis_function,  # Basis function selection
                sli=sli,
                verbose=True
            )
        else:
...
# ======== EVALUTION ===========
# Fix: only evaluate files generated in the current run to avoid cache issues
if args.test:
    # Test mode: only read files generated by the current method run
    # Note: result file naming pattern is {service}_{metric}_{case}.json, not {method}_*.json
    # so we still scan all files, but we only evaluate those for the current data_paths
    rps = []
    ...
    for fault in faults:
        ...
        for rp in rps:
            ...
            for i, ranks in data.items():
                # Filter out IP-address-like strings (e.g., '192-168-29-237-9100')
                s_ranks = []
                for x in ranks:
                    service_name = x.split("_")[0].replace("-db", "")
                    # Check if it matches an IP-address-like pattern (e.g., 192-168-xx-xx-xxxx)
                    is_ip_format = (
                ...
                if old_s_ranks:  # Ensure list is not empty
                ...
                for x in ranks:
                    ...
                    if is_ip_format:
                        continue  # Skip IP-address-like strings
...
overall_metrics = {}
metric_names = ['precision@1', 'precision@3', 'precision@5', 'avg@5']
...
# Debug info: check evaluator status
print(f"🔍 Debug info:")
print(f"  s_evaluator_all cases: {s_evaluator_all.num}")
print(f"  f_evaluator_all cases: {f_evaluator_all.num}")
print(f"  number of evaluated result files: {len(rps)}")
print(f"  evaluated result files: {[basename(rp) for rp in rps]}")
print(f"  number of data_paths: {len(data_paths)}")
...
for name, s_evaluator, f_evaluator in fault_evaluators:
    ...
    # Display key metrics
    key_metrics = ['precision@1', 'precision@3', 'precision@5', 'avg@5']
